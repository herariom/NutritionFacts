...
class NutritionFacts:
    ...
    def process_text(self, text):
        ...
        regex = re.compile(r'calories\s+[0-9]+', re.IGNORECASE)
        facts['Calories'] = self.validate_search(regex, text)

        regex = re.compile(r'total fat\s+[0-9]+', re.IGNORECASE)

        # Increase chances of getting good data by adding types of fat together
        facts['Fat'] = self.process_fat(["saturated fat", "trans fat", "unsaturated fat", "mono.unsaturated fat",
                                         "poly.unsaturated fat"], text)
